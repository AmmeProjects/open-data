...
from amme_open_data.api.ocpi import Location
from amme_open_data.api.ocpi import EVSE
from amme_open_data.api.ocpi import Connector
from amme_open_data.api.ocpi import GeoLocation
from amme_open_data.api.ocpi import ParkingType
from amme_open_data.api.ocpi import Capabilities
from amme_open_data.api.ocpi import ConnectorStandards
from amme_open_data.api.ocpi import ConnectorFormats
from amme_open_data.api.ocpi import PowerTypes
from amme_open_data.api.container import DataContainer
from amme_open_data.conversion.datex2 import DatexCapabilities
from amme_open_data.conversion.datex2 import DatexConnectorFormats
from amme_open_data.conversion.datex2 import DatexPowerTypes
from amme_open_data.conversion.datex2 import DatexConnectorStandards
from amme_open_data.conversion.datex2 import DatexParkingType
...
def parse_connector(doc) -> Connector:
    ...
    return Connector(
        ...
        max_amperage=int(float(doc["egi:maximumCurrent"]))
        if "egi:maximumCurrent" in doc
        else None,
        max_electric_power=int(float(doc["egi:maxPowerAtSocket"]))
        if "egi:maxPowerAtSocket" in doc
        else None,
        last_updated="",
    )
...
def parse_location(doc) -> Location:
    ...
    address = doc["fac:locationReference"]["loc:_locationReferenceExtension"][
        "loc:facilityLocation"
    ]["locx:address"]["locx:addressLine"][0]["locx:text"]["com:values"]["com:value"][
        "#text"
    ]
    city = doc["fac:locationReference"]["loc:_locationReferenceExtension"][
        "loc:facilityLocation"
    ]["locx:address"]["locx:addressLine"][1]["locx:text"]["com:values"]["com:value"][
        "#text"
    ]
    state = doc["fac:locationReference"]["loc:_locationReferenceExtension"][
        "loc:facilityLocation"
    ]["locx:address"]["locx:addressLine"][2]["locx:text"]["com:values"]["com:value"][
        "#text"
    ]
    ...
    datex_parking_type = (
        DatexParkingType(doc.get("egi:typeOfSite"))
        if "egi:typeOfSite" in doc
        and doc.get("egi:typeOfSite") in _enum_values(DatexParkingType)
        else None
    )

    last_updated = (
        datetime.fromisoformat(doc["fac:lastUpdated"])
    ...
    location = Location(
        ...
        postal_code=doc["fac:locationReference"]["loc:_locationReferenceExtension"][
            "loc:facilityLocation"
        ]["locx:address"]["locx:postcode"],
        state=":".join(state.split(":")[1:]).strip(),
        country="ESP",
        coordinates=GeoLocation(
            latitude=float(
                doc["fac:locationReference"]["loc:coordinatesForDisplay"][
                    "loc:latitude"
                ]
            ),
            longitude=float(
        ...
        parking_type=ParkingType[datex_parking_type.name]
        if datex_parking_type is not None
        else None,
    ...
    capabilities = doc["egi:energyInfrastructureStation"].get(
...
def parse_nap_data(path_json) -> DataContainer:
    ...
    egilocations = data_dict["egi:energyInfrastructureTable"][
